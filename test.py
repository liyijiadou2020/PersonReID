...
#           'If you want to use low precision, i.e., fp16, '
...
#增加了一个中间目录Dataset_for_ReID
parser.add_argument('--test_dir',default='../Dataset_for_ReID/Market/pytorch',type=str, help='./test_data')
...
print(">>>> DEBUG, testing model: ",opt.name) # DEBUG
...
# opt.use_NAS = config['use_NAS'] #不使用，注释掉
opt.stride = config['stride']
# if 'use_swin' in config:              #不使用swin，注释掉
...
data_dir = test_dir
# 在Windows中需要把线程数改成单数
# 在linux系统中可以使用多个子进程加载数据，而在windows系统中不能。在windows系统中需要将进程数设置为单进程
if opt.multi:
...
def extract_feature(model,dataloaders):
    ...
    if opt.linear_num <= 0:
        if opt.use_swin or opt.use_swinv2 or opt.use_dense or opt.use_convnext:
            ...
            opt.linear_num = 2048
    total_count = len(dataloaders)
    print(">>> DEBUG, TOTAL COUNT SIZE: ", total_count)

    for iter, data in enumerate(dataloaders):
        img, label = data
        n, c, h, w = img.size()
        count += n
        print(">>> DEBUG, extract_feature... count = ", count)
        # ff = torch.FloatTensor(n,opt.linear_num).zero_().cuda() #BUG: 没有cuda！
        ff = torch.FloatTensor(n, opt.linear_num).zero_()  # BUG: 没有cuda！

        if opt.PCB:
            # ff = torch.FloatTensor(n,2048,6).zero_().cuda() # we have six parts #BUG: 没有cuda！
            ff = torch.FloatTensor(n, 2048, 6).zero_()

        for i in range(2):
            if(i==1):
                img = fliplr(img)
            # input_img = Variable(img.cuda()) #BUG: 没有cuda！
...
print('Here I fuse conv and bn for faster inference, '
      'and it does not work for transformers. '
...
print(">>> DEBUG: NO CUDA, model is like: ", model)

# Extract feature
since = time.time()
with torch.no_grad():
    print(">>> DEBUG: Extracting gallery feature...")
    gallery_feature = extract_feature(model, dataloaders['gallery'])
    print(">>> DEBUG: Extracting query feature...")
...
# os.system('python evaluate_gpu.py | tee -a %s'%result) # 不使用GPU
os.system('python evaluate.py | tee -a %s'%result) # 不使用GPU
